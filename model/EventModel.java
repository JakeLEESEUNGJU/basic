...
public class EventModel {
	...
	public ArrayList selectExi(String title) throws Exception {
		...
		String sql = "SELECT e.evt_no, e.evt_title, e.evt_start, e.evt_end, e.evt_rating, "
				+ "e.evt_price, e.evt_detail, ex.exi_dir, l.loc_no " + "FROM event e, exhibition ex, location l "
				+ "WHERE e.evt_no = ex.evt_no AND e.loc_no = l.loc_no " + "AND e.evt_title = '" + title + "' "
				+ "AND e.evt_flag='t'";
	...
	public ArrayList selectPer(String title) throws Exception {
		...
		String sql = "SELECT e.evt_no, e.evt_title, e.evt_start, e.evt_end, "
				...
				+ "FROM event e, performance p, location l " + "WHERE e.evt_no = p.evt_no AND e.loc_no = l.loc_no "
				+ "AND e.evt_title = '" + title + "' " + "AND e.evt_flag='t'";
		...
		// 시작시간얻어오기
		String sqlStime = "SELECT to_char(per_start, 'hh24mi') per_start " + " FROM performance WHERE evt_no=?";
		...
		// 종료시간얻어오기
		String sqlEtime = "SELECT to_char(per_end, 'hh24mi') per_end " + " FROM performance WHERE evt_no=?";
	...
	public void insertExi(Exhibition vo, String loc) throws Exception {
		...
		String sqlEvt = "INSERT INTO event"
				...
				+ "VALUES (seq_evt_no.nextval, ?, ?,?, ?,?,?, " + "(select loc_no from location where loc_map = '" + loc
				+ "'), 'e')";
	...
	public void insertPer(Performance vo, String loc) throws Exception {
		...
		String sqlEvt = "INSERT INTO event"
				+ "(evt_no, evt_title, evt_start, evt_end, evt_rating, evt_price, evt_detail, loc_no, evt_kind) "
				+ "VALUES (seq_evt_no.nextval, ?, " + "TO_DATE(?,'YYYY/mm/dd'), " + "TO_DATE(?,'YYYY/mm/dd'), "
		...
		String sqlPer = "INSERT INTO performance(per_no, evt_no, per_actor, per_dir, per_start, per_end) "
				+ "VALUES (seq_per_no.nextval, seq_evt_no.currval, ?,?, " + "TO_DATE(?,'YYYY/mm/dd hh24:mi'), "
	...
	public void modifyExi(Exhibition vo, String loc) throws Exception {
		...
		String sql2 = "UPDATE event SET evt_title=?, evt_start=?, evt_end=?, evt_rating=?, "
				+ "evt_detail=?, evt_price=?, " + "loc_no= (SELECT loc_no FROM location WHERE loc_map = '" + loc + "') "
				+ "WHERE evt_title=?";
